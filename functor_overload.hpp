...
namespace mymd {
	namespace detail_fo	{
		...
		template <typename...>	struct reverse_imple;
		
		template <typename... result, typename first, typename... tail>
		struct reverse_imple<types<result...>, first, tail...> : reverse_imple<types<first, result...>, tail...>
		{	};

		template <typename... result>
		struct reverse_imple<types<result...>>		{
			using type = types<result...>;
		};

		template <typename... T>
		using reverse = typename reverse_imple<types<>, T...>::type;
		//-----------------------------------------------------
		...
		template <typename>	struct plus_t_imple;

		template <typename first, typename... tail>
		struct plus_t_imple<types<first, tail...>>	{
			using type = decltype(std::declval<typename plus_t_imple<types<tail...>>::type>() + std::declval<first>());
		};

		template <typename first>
		struct plus_t_imple<types<first>>	{
			using type = first;
		};

		template <typename... T>
		using plus_t = typename plus_t_imple<reverse<T...>>::type;
	...
	using detail_fo::plus_t;	//  functor type generated by operator +
